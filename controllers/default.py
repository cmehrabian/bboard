...
def edit():
    ...
    if p.user_id != auth.user_id:
        session.flash = T("Not authorized to Edit")
        redirect(URL('default', 'index'))
    form = SQLFORM(db.bboard, record=p, readonly=True)
    if form.process().accepted:
...
def index2():
    ...
    def generate_del_button(row):
        #If the record is ours, we can delete
        b = ''
    ...
    form = SQLFORM.grid(q,
        fields =[db.bboard.title, db.bboard.user_id, 
                db.bboard.date_posted, db.bboard.category, db.bboard.bbmessage],
        editable=False, 
        deletable=False,
        links=links,
        paginate = 5,
